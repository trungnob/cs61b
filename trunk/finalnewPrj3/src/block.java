...
public class block implements Comparable<block> { 
	...
	private short ID; // ID is the identification number of block which help to print out nice board
	public boolean isOk(){
		return (block_shape.isOk() && block_pos.isOk());
	}
	...
	public String toStringCoordinate(){
		String temp= block_pos.getRow()+" "+ block_pos.getCol();
		return temp;
	}
	public block(){
	...
	public block(short length, short width, short row, short col){
		block_shape=new BlockShape(length,width);
		block_pos=new Coordinate(row,col);
	}
	
	public block(short length, short width, short row, short col, short IDnum){
...
//		return this.ID==((block)obj).ID;
	...
	public short get_block_row(){
		return block_pos.getRow();
	}
	
	public short get_block_col(){
		return block_pos.getCol();
	}
	
	public short get_block_len(){
		return block_shape.getLength();
	}
	
	public short get_block_wid(){
...
public void MoveTo(short Row, short Col){
getCoor().setCol(Col);
getCoor().setRow(Row);
}
public void MoveLeft(){
MoveTo(get_block_row(),(short)(get_block_col()-1));
}
public void MoveRight(){
MoveTo(get_block_row(),(short)(get_block_col()+1));
}
public void MoveUp(){
MoveTo((short)(get_block_row()-1),get_block_col());
}
